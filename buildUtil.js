...
buildUtil.DojoBuildOptions = {
	...
	"closure": {
		defaultValue: false,
		helpText: "Generate a special build that can be processed by the Closure Compiler with Advanced Optimizations!"
	},
	// CLOSURE: END
	"layerOptimize": {
...
buildUtil.includeLoaderFiles = function(/*String*/dojoLoader, /*String or Array*/hostenvType, /*String*/buildscriptsPath){
	...
	//CLOSURE:BEGIN
	dojo._loadedUrls.push(buildscriptsPath + (kwArgs.closure ? "jslib/dojoGuardStart_Closure.jsfrag" : "jslib/dojoGuardStart.jsfrag"));
	//CLOSURE:END
	...
	if(layerCount){
		...
		for(var i = 0; i < endCount; i++){
			...
			if(layerName == "dojo.js" || layerName == "dojo.xd.js"){
				//CLOSURE:BEGIN
				depList.push(buildscriptsPath + (kwArgs.closure ? "jslib/dojoGuardEnd_Closure.jsfrag" : "jslib/dojoGuardEnd.jsfrag"));
				//CLOSURE:END
			}
...
buildUtil.createLayerContents = function(
	...
	// CLOSURE: BEGIN
	// Do special kind of dojo.provide expansion for Closure
	if(kwArgs.expandProvide && !kwArgs.closure){
	// CLOSURE: END
	...
	if (kwArgs.closure) 
	{
		...
		// Replace (dojo|d).getObject("...", true, dojo) calls (used temporarily in AMD modules)
		// with dojo.provide("dojo...")
		
		dojoContents = dojoContents.replace(/\b(dojo|d|_d)\.getObject\(\s*[\"\']([\w\.]+)[\"\']\s*\,\s*true\s*\,\s*((?:dojo|dijit|dojox)[\w\.]*)\);/mg, function(s, d0, p1, p2) 
		{
			return d0 + ".provide(\"" + p2 + "." + p1 + "\");";
		});

		// Try to eliminate passing method by string name to (dojo|this).connect (which requires an extern):
		//
		//   dojo.connect(obj, "event", scope, "method") ==> dojo.connect(obj, "event", scope, scope.method) 
		//   this.connect(obj, "event", "method") ==> this.connect(obj, "event", this.method)
		...
		// Automatically process all string-based property names prefxed by the comment "/*remap*/" 
		// by calling closureGetMappedPropertyName()

		dojoContents = dojoContents.replace(/\/\*\s*remap\s*\*\/\s*[\'\"](\w+)[\'\"]/img, function(s, fixcomment)
		{
			return 'closureGetMappedPropertyName({' + fixcomment + ':null})';;
		});

		// Try to eliminate passing method by string name to (dojo|this).connect (which requires an extern):
		//
		//   dojo|this.connect(obj, "event", ... ==> dojo|this.connect(obj, closureGetMappedPropertyName({event:null}), ... 
		//
				
		// Note: Complicated connect calls will not be matched -- too difficult to do without actually
		//       parsing the JavaScript.

		// Beware: Event names that are all lower-case and beginning with "on" are NOT processed.  
		//         This is to make sure that most normal DOM events are passed through intact.
		//         Consequently, all-lower-case custom events are also NOT processed.
		//
		//         The user should be following the Dojo style guild and name his/her events something
		//         like "onXxxYyy" (e.g. onChange, onClick etc.)  If he/she names an event "onfluctuate",
		//         for example, it will be assumed a DOM event and not processed.  That event should
		//         be named "onFluctuate" instead.
		//
		//         The only situation where it should become a problem is when a user-defined event/method
		//         is one single word (therefore no capitalization of the second word) and the word 
		//         just HAPPENS to begin with "on" (e.g. only, one, onion etc.)
		//  
		//         In these strange cases, we don't really case.  If a user uses careless names like 
		//         these and expects to compile the program with the Closure Compiler in ADVANCED 
		//         OPTIMIZATIONS mode, then he/she DESERVES to have it done to him/her!
		//
		//         Also, events beginning with "webkit" and has capitalized words following are
		...
		dojoContents = dojoContents.replace(/\b(dojo|d|_d|this|_this)\.connect\s*\(\s*([\w\.]+)\s*\,\s*[\'\"](\w+)[\'\"]/mg, function(s, d0, obj, evt)
		{
			...
			// Filter out normal DOM events -- all lower-case and beginning with "on"
			if (evt.indexOf("on") === 0 && evt.toLowerCase() === evt) return s;			
			
			// Filter out WebKit-specific events -- mixed-case and beginning with "webkit"
			if (evt.indexOf("webkit") === 0 && evt.toLowerCase() !== evt) return s;		

			return d0 + '.connect(' + obj + ', closureGetMappedPropertyName({' + evt + ':null})';
		});
		
		// Try to eliminate passing method by string name to (dojo|this).subscribe (which requires an extern):
		//
		//   dojo.subscribe(topic, scope, "method") ==> dojo.subscribe(topic, scope, scope.method) 
		//   this.subscribe(topic, "method") ==> this.subscribe(topic, this.method)
		...
		// Try to eliminate passing method by string name to dojo.hitch (which requires an extern):
		//
		//   dojo.hitch(scope, "method", ...) ==> dojo.hitch(scope, scope.method, ...) 
		
		// Note: Complicated hitch calls will not be matched -- too difficult to do without actually
		//       parsing the JavaScript.

		dojoContents = dojoContents.replace(/\b(dojo|d|_d)\.hitch\s*\(\s*([\w\.]+)\s*\,\s*[\'\"](\w+)[\'\"]/mg, function(s, d0, scope, mtd) 
		{
			switch(scope) {
				case "null": return d0 + ".hitch(" + scope + ", " + mtd;
				default:     return d0 + ".hitch(" + scope + ", " + scope + "." + mtd;
			}
		});
		
		// Replace browser sniffing variables with global variables of the same name (but upper-cased)
		// so that we can provide constant values (if possible) and the Clousure Compiler will then
		// elminate all unnecessary browser code.  For example, all references to "dojo.isWebKit" or
		// "d.isWebKit" will be replaced simply by "ISWEBKIT".
		
		// Need to define top-level constants (perhaps in dojoGuardStart.jsfrag):
		//   var ISIE, ISFF, ISWEBKIT, ISOPERA, ISKHTML, ISCHROME, ISMAC, ISMOZ, ISMOZILLA, ISSAFARI
			
		dojoContents = dojoContents.replace(/\b(?:dojo|d|_d)\.(isIE|isFF|isAIR|isWebKit|isOpera|isKhtml|isChrome|isMac|isMoz|isMozilla|isSafari)\b/mg, function(s, p1) 
		{
		...
		//   dojo.declare("foo.bar.Baz", [ hello.world, abc.def.xyz ], { ...
		// ==>
		//	 /** @constructor 
		//    *  @extends {hello.world}
		//    *  @param {...Object} params
		...
		dojoContents = dojoContents.replace(declareRegExp, function(s, d0, p1, p2) 
		{
			...
			if (!p2 || p2.toLowerCase() === "null") p2 = null;
		...
		// Convert dojo.provide("foo.bar.Baz") ==> goog.provide("foo.bar.Baz")
		// Put all goog.provide statments at the beginning of the file

		// Note: DO NOT set dojo._loadedModules["foo.bar.Baz"] to the namespace object if intending  
		//       to use the Closure Compiler because it will create an "alias" and prevent optimization
		//       of the entire namespace!
		
		// Beware: This will break certain code (such as i18n).  i18n modules are loaded
		//         (stupidly) via dojo._loadedModules. For such cases, manually set in user code:
		//
		//           dojo._loadedModules["foo.bar.Baz"] = foo.bar.Baz;
		//		
		//         BEFORE loading i18n (or any module that depends on i18n, needless to say).

		rawtext = buildUtil.removeComments(dojoContents);
		var googstatements = [];
		
		dojoContents = dojoContents.replace(/\bdojo\.provide\s*\(\s*[\"\']([\w\.]*)[\"\']\s*\)/mg, function(s, p1)
		{
			if (rawtext.indexOf(s) >= 0) {
				var parts = p1.split(".");

				var regexp = new RegExp("\\b" + p1.replace(".", "\\.") + "\\s*\\=", "mg");
				
				for (var x = 0; x < parts.length; x++) {
					var parent = parts.slice(0,x+1).join(".");
					
					if (googprovides.indexOf(parent) >= 0) continue;
					
					if (x < parts.length - 1 || !regexp.test(rawtext)) {
						// Only provide for objects that will not be overridden later on
						googprovides.push(parent);
						googstatements.push('goog.provide("' + parent + '");');
					} 
				}

				return 'closureEmpty("' + p1 + '")';
			} else {
				return s;
			} 
		});

		// Create property name maps:
		//   Any class variable declaration preceeded with "/*public*/" (whitespace insignificant)
		//   will be stored, and a "property names map" placed where there is "/*propnamemap*/".

		//   A variable can be added to the property name map via the comment "/*public(varname)*/",
		//   e.g.: /*public(value)*/ to implement the "value" property without declaring a property.

		//   Also, any property declaration which is "_setXxxAttr: function" or "_getXxxAttr: function"
		//   is automatically added to the property name map.

		// Beware: The property name map will end with a comma, so make sure there is something AFTER
		//         /** @propnamemap */ in the class declaration!

		var fullmap = {};
		var map = [];

		dojoContents = dojoContents.replace(/\/\*\s*public\s*\(\s*(\w+)\s*\)\s*\*\/|\/\*\s*public\s*\*\/\s*(\w+)/img, function(s, p0, p1) 
		{
			if (p0) {
				map.push(p0);
				fullmap[p0] = p0;
				//print ("> Found public property " + p0);
				return "";
			}
			if (p1) {
				map.push(p1);
				fullmap[p1] = p1;
				//print ("> Found public property " + p1);
				return p1;
			}
		});

		// Put in the closurePropertyNamesMap and goog.provide's 		

		var mapstmt = "";

		for (var name in fullmap) {
			if (!fullmap.hasOwnProperty(name)) continue;
			if (mapstmt) mapstmt += "," + lineSeparator;
			mapstmt += "\t" + name + ':"' + name + '"';

			var uc = name.charAt(0).toUpperCase() + name.substr(1); 
			var nname = uc + "Node";
			var gname = "_get" + uc + "Attr";
			var sname = "_set" + uc + "Attr";

			if (rawtext.indexOf(nname) >= 0) mapstmt += ', ' + nname + ':"' + nname + '"';
			if (rawtext.indexOf(gname) >= 0) mapstmt += ', ' + gname + ':"' + gname + '"';
			if (rawtext.indexOf(sname) >= 0) mapstmt += ', ' + sname + ':"' + sname + '"';
		}
		...
		var gpmarkerloc = dojoContents.indexOf(gpmarker);
		if (gpmarkerloc <= 0) {
			dojoContents = googstatements.join(lineSeparator) + lineSeparator + lineSeparator + dojoContents;
		} else {
			dojoContents = dojoContents.substring(0,gpmarkerloc) + googstatements.join(lineSeparator) + lineSeparator + dojoContents.substring(gpmarkerloc + gpmarker.length);
		}
		...
		// It is too common to have function declarations or function calls inside a comment block, so 
		// we must filter out all comments before doing this, otherwise the JsDoc comments will 
		...
		// Then remove all comments
		rawtext = buildUtil.removeComments(markedtext);

		// Find all the function declarations and calls left
		while((tmp = /__X_X_X_XXX_X_X_X__\d+/mg.exec(rawtext)) != null) 
		{
			functions[tmp[0]] = true;
		}

		// Rewrite dojo.extend/mixin
		//   dojo.extend(foo.bar.Baz, { ... }
		// ==>
		//   dojo.extend(foo.bar.Baz, /** @lends {foo.bar.baz} */ { ... }

		//   dojo.mixin(foo.bar.Baz, { ... }
		// ==>
		//   dojo.mixin(foo.bar.Baz, /** @lends foo.bar.baz */ { ... }
			
		// Note: Complicated dojo.extend and dojo.mixin expressions are not rewritten, such as: 
		//       dojo.extend(something[i]...
		//       This should be enough to catch 99% of the cases for dojo.extend as it is normally used
		//       to extend classes.
		
		var mixinRegExp = /(__X_X_X_XXX_X_X_X__\d+)(dojo|d|_d)\.(extend|mixin)\s*\(\s*([\w\.]+\s*\,)/mg;
		
		dojoContents = markedtext.replace(mixinRegExp, function(s, mp, d0, p1, p2) 
		{
			if (!(mp in functions)) return "*** NOT PROCESSED *** " + s.substring(mp.length);		// Inside a comment

			var classname = p2.replace(/\s+/g, "").slice(0,-1);	// Get rid of the ending comma
			
			return d0 + "." + p1 + "(" + classname + ", /** @lends " + (p1 === "extend" ? classname + ".prototype" : classname) + " */ ";  
		});

		// Create JsDoc for functions with Dojo-style parameter types documentation:
		//   function (/*Number|String*/foo, /*Object|DOMNode?*/bar /*, ...*/) ...
		// ==>
		//   /** @param {number|string} foo
		//    *  @param {*} bar
		//    *  @param {object|Node=} baz
		//    *  @param {...} v_args
		//    */
		//   function (/*Number|String*/foo, /*anything*/bar, /*Object|DOMNode?*/baz, v_args) ...
		
		// Note: Common Dojo-style types are recognized (e.g. integer/int/number/float are all 
		//       mapped to "number", anything is mapped to "*" etc.).
		//       However, it may be a good idea for Dojo to standardize on the type names, although
		//       for Dojo it is simply documentation, but having good type names enables the 
		...
		dojoContents = dojoContents.replace(jsDocRegExp, function(s, v0, mp, f0, p1, p2) 
		{
			...
			if (!(mp in functions)) return (v0 || "") + s.substring(funcpos);
			...
			for (var x = 0; x < params.length; x++) {
				var par = params[x].replace(/\s+/mg,"");
				var match = /\/\*(.*)\*\/\s*(\w+)/.exec(par);
				if (!match) {
					// No comment -- map to {*}
					stmt += (!stmt ? "/** " : " *  ");
					stmt += "@param {*} " + par + lineSeparator;
					continue;
				}
				...
				for (var y = 0; y < types.length; y++) 
				{
					...
					{
						case "nothing":
						case "null": mapped = "null"; break;
						
						case "undefined": mapped = "undefined"; break;
						...
						case "boolean": mapped = "boolean"; break;
						case "array":  mapped = "Array"; break;
						case "function": mapped = "Function"; break;

						case "regex":
						case "regexp": mapped = "RegExp"; break;
						
						case "node":
						case "domnode": mapped = "Node"; break;
						
						case "nodelist": mapped = "NodeList"; break;
						...
						// dojo._base.event types
						
						case "event": mapped = "Event"; break;

						case "mouseevent": mapped = "MouseEvent"; break;

						case "keyboardevent": mapped = "KeyboardEvent"; break;

						// dojo._base.xhr types
												
						case "xhr":
						case "xmlhttprequest": mapped = "XMLHttpRequest"; break;
						
						// dojo.data types
						
						case "attribute-name-string": mapped = "string"; break;

						case "item": mapped = "Object"; break;

						case "almostanything":
						case "value": mapped = "*"; break;

						// dijit types
						
						case "_widget":
						case "widget": mapped = "dijit._Widget"; break;
				...
				stmt += (!stmt ? "/** " : " *  ");
				stmt += "@param {" + jsdoc + (optional ? "=" : "") + "} " + pname + lineSeparator;
				typecount++;
			}

			if (typecount <= 0) return (v0 || "") + s.substring(funcpos);

			if (p2) stmt += " *  @param {...} v_args" + lineSeparator;
			stmt += " */ ";
			
			//print ("!!! JsDoc = " + stmt);
			return lineSeparator + stmt + lineSeparator + (v0 || "") + s.substring(funcpos);
		});
	}
	// CLOSURE: END
	...
	if(fileList){
		for(i = 0; i < fileList.length; i++){
			...
			if(match){
				...
				// Change dojo._hasResource["foo.bar.Baz"] guards to:
				// dojo.global.__dojo$_hasResource$foo$bar$Baz$ in order to facilitate namespace flattening
				// by the Closure Compiler

				if (kwArgs.closure) {
					var pmatch = match[1].slice(1,-1).replace(/\./g,"$");
					
					existingGuardRegExp = new RegExp('if\\(\\!dojo\\.global\\.__dojo\\$_hasResource\\$' + pmatch.replace(/\$/g,"\\$") + '\\$\\)');
					
					if(!fileContents.match(existingGuardRegExp)){
						fileContents = 'if(!dojo.global.__dojo$_hasResource$' + pmatch + '$) {'
							+ lineSeparator
							+ 'dojo.global.__dojo$_hasResource$' + pmatch + '$ = true;'
							+ lineSeparator
							+ fileContents
							+ lineSeparator
							+ '}'
